...
def test_orchestrator_does_not_refresh_account_state_after_rejected_execution(monkeypatch):
    class _StubExchange:
        def __init__(self, dry_run, shadow_mode):
            self.dry_run = dry_run
            self.shadow_mode = shadow_mode
    ...
    settings = _settings(universe_symbols=["ETH"], dry_run=False, shadow_mode=False)
    orchestrator = orchestrator_module.Orchestrator(settings)

    orchestrator.run_once()

    assert orchestrator.exchange.account_state_calls == 1
