...
def _fmt_compact(value: Any, fallback: str = "n/a", max_len: int = 800) -> str:
    if value in (None, "", [], {}, ()):
        return fallback
    text = str(value)
    if len(text) <= max_len:
        ...
        for asset in self.settings.universe_symbols:
            ...
            if gate.allowed and gate.final_action not in {"NO_TRADE", "HOLD"}:
                ...
                if execution_accepted:
                    ...
                    logger.error(
                        "❌ %s | Execution result | accepted=%s | dry_run=%s | order_status=%s | error=%s | sent_to_exchange=%s | mode=%s | response=%s",
                        asset,
                        execution_accepted,
                        execution_result.get("dry_run", self.settings.dry_run),
                        execution_order_status,
                        ...
                        _fmt_compact(execution_result.get("response_excerpt") or execution_result.get("response")),
                    )
                if execution_accepted:
                    account_state = self.exchange.get_account_state()
